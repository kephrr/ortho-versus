const fs = require('fs');
const content = import ffmpeg from 'fluent-ffmpeg';
...
export async function sliceAndNormalize(rawAudioPath: string, targetDuration: number, outputDir: string): Promise<string[]> {
  console.log(\[AudioProcessor] Analyse des silences pour \...\);
  ...
  const totalDuration = await new Promise<number>((resolve, reject) => {
    ffmpeg.ffprobe(rawAudioPath, (err: Error | null, data: any) => {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const outPath = path.join(outputDir, \\_seg\.mp3\);
    console.log(\[AudioProcessor] Extraction du segment \ (\s -> \s)...\);
...
;
fs.writeFileSync('src/services/audioProcessor.ts', content);
